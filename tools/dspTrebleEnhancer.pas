     ...
     *********************************************************************)
{
  @abstract(DSP Filter to Amplify High Frequencys.)
  @author(Milenko Mitrovic <dcoder@dsp-worx.de>)
  @created(Sep 01, 2002)
  @lastmod(Apr 02, 2004)
}

unit dspTrebleEnhancer;

interface

uses
  Classes, dspConst, dspUtils, Math, dspInterfaces, dspWindows;

type
  { TDCTrebleEnhancer - DSP Component to Amplify High Frequencys of Audio Data. The
    Frequencyrange can be adjusted. }
  TDCTrebleEnhancer = class(TComponent, IDCTrebleEnhancer)
  private
    {@exclude}
    fSeperate : Boolean;
    {@exclude}
  ...
  public
    { TDCTrebleEnhancer constructor }
    constructor Create(AOwner: TComponent); override;
    { TDCTrebleEnhancer destructor }
  ...
  published
    ...
    { Specifys the Frequency Range that will be used to Amplify. Range is Frequency
      to (Samplerate div 2). }
...
constructor TDCTrebleEnhancer.Create(AOwner: TComponent);
var
  i : integer;
begin
...
destructor TDCTrebleEnhancer.Destroy;
begin
  inherited Destroy;
end;

procedure TDCTrebleEnhancer.SetVolume(Channel : Byte; Volume : Word);
begin
  if not InRange(Channel,0,MaxChannels -1) then Exit;
  fVolume[Channel] := Volume;
end;

function TDCTrebleEnhancer.GetVolume(Channel : Byte) : Word;
begin
  Result := 0;
  if not InRange(Channel,0,MaxChannels -1) then Exit;
...
procedure TDCTrebleEnhancer.Process(Buffer : Pointer; Size : Integer; SampleRate : integer; Bits : Byte; Channels : Byte; Float : Boolean);
var
  SplitBuffer : PChar;
  SizeLeft : integer;
  SplitSize : integer;
...
begin
  ...
  begin
    ...
    begin
      ...
      {$IFNDEF FPC}if fProcessMessages then Application.ProcessMessages;{$ENDIF}
      dec(SizeLeft,SplitSize);
    end;
  end;
end;

procedure TDCTrebleEnhancer.DoDSP(Buffer : Pointer; Size : Integer; Bits : Byte; Channels : Byte; Float : Boolean);
var
  Buf8  : PByteArray;
    ...
    begin
      ...
      begin
        ...
        begin
          ...
          Buf8^[i * Channels + c] := Clip_8(Round(fPreviousOutput[2][c] * Vol) + (Buf8^[i * Channels + c] - 128)) + 128;
        end;
      end;
    end;
    16:
    begin
      ...
      begin
        ...
        begin
          ...
          fPreviousOutput[2][c] := CurrSample - fPreviousOutput[0][c] - fPreviousOutput[1][c];
          fPreviousOutput[1][c] := fWorkFreq * fPreviousOutput[2][c] + fPreviousOutput[1][c];
          Buf16^[i * Channels + c] := Clip_16(Round(fPreviousOutput[2][c] * Vol) + Buf16^[i * Channels + c]);
        end;
      end;
    end;
    24:
    begin
      ...
      begin
        ...
        begin
          ...
          fPreviousOutput[2][c] := CurrSample - fPreviousOutput[0][c] - fPreviousOutput[1][c];
          fPreviousOutput[1][c] := fWorkFreq * fPreviousOutput[2][c] + fPreviousOutput[1][c];
          Buf24^[i * Channels + c] := Cvt32BitTo24(Clip_24(Round(fPreviousOutput[2][c] * Vol) + Cvt24BitTo32(Buf24^[i * Channels + c])));
        end;
      end;
    end;
    32:
      ...
      begin
        ...
        begin
          ...
          begin
            ...
            fPreviousOutput[2][c] := CurrSample - fPreviousOutput[0][c] - fPreviousOutput[1][c];
            fPreviousOutput[1][c] := fWorkFreq * fPreviousOutput[2][c] + fPreviousOutput[1][c];
            Buf32^[i * Channels + c] := fPreviousOutput[2][c] * Vol + Buf32^[i * Channels + c];
          end;
        end;
      end else
      begin
        ...
        begin
          ...
          begin
            ...
            fPreviousOutput[2][c] := CurrSample - fPreviousOutput[0][c] - fPreviousOutput[1][c];
            fPreviousOutput[1][c] := fWorkFreq * fPreviousOutput[2][c] + fPreviousOutput[1][c];
            Buf32i^[i * Channels + c] := Clip_32(Int64(Round(fPreviousOutput[2][c] * Vol) * 32768) + Buf32i^[i * Channels + c]);
          end;
        end;
      end;
    end;
...
procedure TDCTrebleEnhancer.SetFrequency(Frequency : integer);
begin
...
procedure TDCTrebleEnhancer.Init(SampleRate : integer);
begin
...
procedure TDCTrebleEnhancer.Flush;
begin
  FillChar(fPreviousOutput,SizeOf(TPass),0);
end;
(*** IDCTrebleEnhancer ********************************************************)
function TDCTrebleEnhancer.get_Enabled(out aEnabled: BOOL): HRESULT; stdcall;
begin
  Result := S_OK;
  aEnabled := fEnabled;
end;

function TDCTrebleEnhancer.set_Enabled(aEnabled: BOOL): HRESULT; stdcall;
begin
  Result := S_OK;
  fEnabled := aEnabled;
end;

function TDCTrebleEnhancer.get_Seperate(out aSeperate: BOOL): HRESULT; stdcall;
begin
  Result := S_OK;
  aSeperate := fSeperate;
end;

function TDCTrebleEnhancer.set_Seperate(aSeperate: BOOL): HRESULT; stdcall;
begin
  Result := S_OK;
  fSeperate := aSeperate;
end;

function TDCTrebleEnhancer.get_Volume(aChannel: Byte; out aVolume: Word): HRESULT; stdcall;
begin
  Result := S_OK;
  aVolume := GetVolume(aChannel);
end;

function TDCTrebleEnhancer.set_Volume(aChannel: Byte; aVolume: Word): HRESULT; stdcall;
begin
  Result := S_OK;
  SetVolume(aChannel,aVolume);
end;

function TDCTrebleEnhancer.get_Frequency(out aFrequency: integer): HRESULT; stdcall;
begin
  Result := S_OK;
  aFrequency := fFrequency;
end;

function TDCTrebleEnhancer.set_Frequency(aFrequency: integer): HRESULT; stdcall;
begin
  Result := S_OK;
