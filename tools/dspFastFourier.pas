...
uses
  Classes, Math, dspConst, dspUtils, dspWindows;
...
var
  d32 : packed array[0..6] of TComplex;
  d64 : packed array[0..14] of TComplex;
  d128 : packed array[0..30] of TComplex;
  d256 : packed array[0..62] of TComplex;
  d512 : packed array[0..126] of TComplex;
  d1024 : packed array[0..126] of TComplex;
  d2048 : packed array[0..254] of TComplex;
  d4096 : packed array[0..510] of TComplex;
  d8192 : packed array[0..1022] of TComplex;
  ...
  {$IFNDEF FPC}
  SqrtHalf3DNow : packed array[0..1] of Single = (
    0.70710678118654, 0.70710678118654
  );

  d16re3DNow : packed array[0..1] of Single = (
    0.92387953251128, 0.92387953251128
  );

  d16im3DNow : packed array[0..1] of Single = (
    0.38268343236508, 0.38268343236508
  );

  Invert3DNow_H : packed array[0..1] of Cardinal = (
    $80000000, 0
  );
  {$ENDIF}
  ...
  SqrtHalfSSE : packed array[0..3] of Single = (
    0.70710678118654, 0.70710678118654, 0.70710678118654, 0.70710678118654
  );

  d16reSSE : packed array[0..3] of Single = (
    0.92387953251128, 0.92387953251128, 0.92387953251128, 0.92387953251128
  );

  d16imSSE : packed array[0..3] of Single = (
    0.38268343236508, 0.38268343236508, 0.38268343236508, 0.38268343236508
  );

  InvertSSE_HL : packed array[0..3] of Cardinal = (
    $80000000, 0, 0, $80000000
  );

  InvertSSE_MHL : packed array[0..3] of Cardinal = (
    0, $80000000, $80000000, 0
  );

  InvertSSE_LL : packed array[0..3] of Cardinal = (
    0, 0, $80000000, $80000000
  );

procedure irbtCopyFromTo(pFrom, pTo : PWORDArray; x,z,m : PInteger);
var
  i : integer;
begin
...
begin
  ...
  irbtCopyFromTo(@RevBins2,@RevBins4,@x,@z,@m);
  irbtCopyFromTo(@RevBins4,@RevBins8,@x,@z,@m);
  irbtCopyFromTo(@RevBins8,@RevBins16,@x,@z,@m);
  irbtCopyFromTo(@RevBins16,@RevBins32,@x,@z,@m);
  irbtCopyFromTo(@RevBins32,@RevBins64,@x,@z,@m);
  irbtCopyFromTo(@RevBins64,@RevBins128,@x,@z,@m);
  irbtCopyFromTo(@RevBins128,@RevBins256,@x,@z,@m);
  irbtCopyFromTo(@RevBins256,@RevBins512,@x,@z,@m);
  irbtCopyFromTo(@RevBins512,@RevBins1024,@x,@z,@m);
  irbtCopyFromTo(@RevBins1024,@RevBins2048,@x,@z,@m);
  irbtCopyFromTo(@RevBins2048,@RevBins4096,@x,@z,@m);
  irbtCopyFromTo(@RevBins4096,@RevBins8192,@x,@z,@m);
end;
