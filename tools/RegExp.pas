...
class function TRegExpr.InvertCaseFunction (const Ch : REChar) : REChar;
 begin
  ...
  {$ENDIF}
   begin
    Result := {$IFDEF DELPHI}AnsiUpperCase (Ch) [1]{$ELSE} REChar (CharUpper (PChar (Ch))){$ENDIF};
    if Result = Ch
     then Result := {$IFDEF DELPHI}AnsiLowerCase (Ch) [1]{$ELSE} REChar (CharLower (PChar (Ch))){$ENDIF};
   end;
  ...
  if PREOp (regnext (scan))^ = EEND then begin // Only one top-level choice.
    ...
    if (flags and SPSTART) <> 0 then begin
        ...
        while scan <> nil do begin
          if (PREOp (scan)^ = EXACTLY)
             and (longint(strlen (scan + REOpSz + RENextOffSz)) >= longint(len)) then begin
