     ...
     *********************************************************************)
{
  @abstract(DSP Filter that cuts Frequencys from a specified Frequency to half
...
type
  { TDCHighpass - DSP Component that cuts Frequencys from a specified Frequency
    to half of the Sampling Rate. Each Channel can be seperate Processed with
    different Cutoff Values. } 
  TDCHighpass = class(TComponent, IDCHighPass)
  private
    {@exclude}
    fSeperate : Boolean;
    {@exclude}
  ...
  public
    { Creates an Instance of TDCHighpass. }
...
constructor TDCHighpass.Create(AOwner: TComponent);
var
  i : integer;
begin
  inherited Create(AOwner);
  for i := 0 to MaxChannels -1 do fCutoff[i] := 0;
  fSeperate := false;
  fProcessMessages := False;
  fSampleSize := DefaultSampleSize;
end;

destructor TDCHighpass.Destroy;
begin
  inherited Destroy;
end;

procedure TDCHighpass.SetCutoff(Channel : Byte; Cutoff : Cardinal);
begin
  if not InRange(Channel,0,MaxChannels -1) then Exit;
  fCutoff[Channel] := Cutoff;
end;

function TDCHighpass.GetCutoff(Channel : Byte) : Cardinal;
begin
  Result := 0;
  if not InRange(Channel,0,MaxChannels -1) then Exit;
...
procedure TDCHighpass.Process(Buffer : Pointer; Size,SampleRate : Integer; Bits : Byte; Channels : Byte; Float : Boolean);
var
  SplitBuffer : PChar;
  SizeLeft : integer;
  SplitSize : integer;
...
procedure TDCHighpass.DoDSP(Buffer : Pointer; Size,SampleRate : Integer; Bits : Byte; Channels : Byte; Float : Boolean);
var
  Buf8  : PByteArray;
    ...
    begin
      ...
      begin
        ...
        begin
          ...
          Buf8^[i * Channels + c] := Clip_8(Round(fPreviousOutput[2][c])) + 128;
        end;
      end;
    end;
    16:
    begin
      ...
      begin
        ...
        begin
          ...
          fPreviousOutput[2][c] := CurrSample - fPreviousOutput[0][c] - fPreviousOutput[1][c];
          fPreviousOutput[1][c] := f * fPreviousOutput[2][c] + fPreviousOutput[1][c];
          Buf16^[i * Channels + c] := Clip_16(Round(fPreviousOutput[2][c]));
        end;
      end;
    end;
    24:
    begin
      ...
      begin
        ...
        begin
          ...
          fPreviousOutput[2][c] := CurrSample - fPreviousOutput[0][c] - fPreviousOutput[1][c];
          fPreviousOutput[1][c] := f * fPreviousOutput[2][c] + fPreviousOutput[1][c];
          Buf24^[i * Channels + c] := Cvt32BitTo24(Clip_24(Round(fPreviousOutput[2][c])));
        end;
      end;
    end;
    32:
      ...
      begin
        ...
        begin
          ...
          begin
            ...
            fPreviousOutput[2][c] := CurrSample - fPreviousOutput[0][c] - fPreviousOutput[1][c];
            fPreviousOutput[1][c] := f * fPreviousOutput[2][c] + fPreviousOutput[1][c];
            Buf32^[i * Channels + c] := fPreviousOutput[2][c];
          end;
        end;
      end else
      begin
        ...
        begin
          ...
          begin
            ...
            fPreviousOutput[2][c] := CurrSample - fPreviousOutput[0][c] - fPreviousOutput[1][c];
            fPreviousOutput[1][c] := f * fPreviousOutput[2][c] + fPreviousOutput[1][c];
            Buf32i^[i * Channels + c] := Clip_32(Int64(Round(fPreviousOutput[2][c]) * 32768));
          end;
        end;
      end;
    end;
...
procedure TDCHighpass.Flush;
begin
  FillChar(fPreviousOutput,SizeOf(TPass),0);
end;
(*** IDCHighPass **************************************************************)
function TDCHighpass.get_Enabled(out aEnabled: BOOL): HRESULT; stdcall;
begin
  Result := S_OK;
  aEnabled := fEnabled;
end;

function TDCHighpass.set_Enabled(aEnabled: BOOL): HRESULT; stdcall;
begin
  Result := S_OK;
  fEnabled := aEnabled;
end;

function TDCHighpass.get_Seperate(out aSeperate: BOOL): HRESULT; stdcall;
begin
  Result := S_OK;
  aSeperate := fSeperate;
end;

function TDCHighpass.set_Seperate(aSeperate: BOOL): HRESULT; stdcall;
begin
  Result := S_OK;
  fSeperate := aSeperate;
end;

function TDCHighpass.get_Cutoff(aChannel : Byte; out aCutoff : Cardinal): HRESULT; stdcall;
begin
  Result := S_OK;
  aCutoff := GetCutoff(aChannel);
end;

function TDCHighpass.set_Cutoff(aChannel : Byte; aCutoff : Cardinal): HRESULT; stdcall;
begin
  Result := S_OK;
